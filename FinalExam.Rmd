...
```
##Graphics
```{r keyplot, echo=TRUE, results=TRUE, warning=FALSE, message=FALSE}
library(ggplot2)
...
table(songs$Time)
```
__2. Why were the remaining variables (ID, Title, Artist, and Time) in the dataset not included as "features" in the model?__
Because ID and Title are not dependent variables but identifiers. They have unique values for each observation so they don't predict anything. There are `r uniqueArtist` unique value for artists from the total 100 observations so they won't really help predict anything. As for time, as the table above shows, most of the songs are 4 minutes except for 1 song which is 3 minutes and that kind of ratio doesn't really give significant predictions.
